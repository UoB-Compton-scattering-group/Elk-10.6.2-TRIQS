...
if (ramdisk) then
  call putrd(fname,ik,v1=vkl(1:3,ik),n1=nmatmax,n2=nstfv,n3=nspnfv, &
   nzv=nmatmax*nstfv*nspnfv,zva=evecfv)
end if
! write to disk if required
if (wrtdsk) then
! find the record length
  inquire(iolength=recl) vkl(1:3,ik),nmatmax,nstfv,nspnfv,evecfv
  open(202,file=fname,form='UNFORMATTED',access='DIRECT',recl=recl)
  write(202,rec=ik) vkl(1:3,ik),nmatmax,nstfv,nspnfv,evecfv
  close(202)
end if
