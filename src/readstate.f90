...
if (any(version(:) /= version_(:))) then
  ...
  write(*,'(" STATE.OUT : ",I0,".",I0,".",I0)') version_
end if
read(100) spinpol_
read(100) nspecies_
...
if ((version_(1) >= 10).or.((version_(1) == 9).and.(version_(2) >= 6))) then
  read(100) efermi
else
  call readfermi
end if
...
do i3=0,ngridg(3)-1
  ...
  do i2=0,ngridg(2)-1
    ...
    do i1=0,ngridg(1)-1
      ...
      ir=i3*ngridg(2)*ngridg(1)+i2*ngridg(1)+i1+1
      mapir(ir)=jr
    end do
  end do
end do
...
rhoir(1:ngtot)=rfir_(mapir(1:ngtot))
! read the Coulomb potential, regrid and pack
read(100) rfmt_,rfir_
call rgfmt(vclmt)
vclir(1:ngtot)=rfir_(mapir(1:ngtot))
! read the exchange-correlation potential, regrid and pack
read(100) rfmt_,rfir_
call rgfmt(vxcmt)
vxcir(1:ngtot)=rfir_(mapir(1:ngtot))
...
call rgfmt(vsmt)
vsir(1:ngtot)=rfir_(mapir(1:ngtot))
! compute the Kohn-Sham potential on coarse interstitial grid
...
! component map for spin-polarised case
  ...
  call rgvfcmt(bsmt)
  call rgvir(bsir)
! compute the Kohn-Sham magnetic field on coarse interstitial grid
  do idm=1,ndmag
...
if (any(xcgrad == [3,4,5,6])) then
  if (any(xcgrad_ == [3,4,5,6])) then
    read(100) rfmt_,rfir_
    call rgfmt(wxcmt)
    wxcir(1:ngtot)=rfir_(mapir(1:ngtot))
  else
    wxcmt(:,:)=0.d0
    wxcir(1:ngtot)=0.d0
  end if
...
do idm=1,ndmag
  jdm=mapidm(idm)
  if (jdm /= 0) then
    rvfir(1:ngtot,idm)=rvfir_(mapir(1:ngtot),jdm)
  else
    rvfir(1:ngtot,idm)=0.d0
  end if
end do
end subroutine
