...
! !ROUTINE: xcifc
! !INTERFACE:
subroutine xcifc(xctype,n,c_tb09,tempa,rho,rhoup,rhodn,grho,gup,gdn,g2rho,g2up,&
 g2dn,g3rho,g3up,g3dn,grho2,gup2,gdn2,gupdn,tau,tauup,taudn,ex,ec,vx,vc,vxup, &
 vxdn,vcup,vcdn,dxdgr2,dxdgu2,dxdgd2,dxdgud,dcdgr2,dcdgu2,dcdgd2,dcdgud,dxdg2r,&
...
case(50)
! Thomas-Fermi kinetic energy functional derivative δτ(r')/δρ(r)
  if (present(rhoup).and.present(rhodn).and.present(dtdru) &
   .and.present(dtdrd)) then
    call k_tf_sp(n,rhoup,rhodn,dtdru,dtdrd)
    if (present(dtdgu2)) dtdgu2(1:n)=0.d0
    if (present(dtdgd2)) dtdgd2(1:n)=0.d0
  else if (present(rho).and.present(dtdr)) then
    call k_tf(n,rho,dtdr)
    if (present(dtdgr2)) dtdgr2(1:n)=0.d0
  else
    goto 10
  end if
case(51)
! Thomas-Fermi kinetic energy functional derivative scaled by the ratio of the
! exact τ to the TF τ
  if (present(rhoup).and.present(rhodn).and.present(tauup).and.present(taudn) &
   .and.present(dtdru).and.present(dtdrd)) then
    call k_tfsc_sp(n,rhoup,rhodn,tauup,taudn,dtdru,dtdrd)
    if (present(dtdgu2)) dtdgu2(1:n)=0.d0
    if (present(dtdgd2)) dtdgd2(1:n)=0.d0
  else if (present(rho).and.present(tau).and.present(dtdr)) then
    call k_tfsc(n,rho,tau,dtdr)
    if (present(dtdgr2)) dtdgr2(1:n)=0.d0
  else
    goto 10
  end if
case(52)
! Thomas-Fermi-von Weizsacker kinetic energy functional derivative
...
! libxc library functionals
  if (present(rhoup).and.present(rhodn).and.present(g2up).and.present(g2dn) &
   ...
   .and.present(vcdn).and.present(dxdgu2).and.present(dxdgd2) &
   .and.present(dxdgud).and.present(dcdgu2).and.present(dcdgd2) &
   .and.present(dcdgud).and.present(dxdg2u).and.present(dxdg2d) &
   .and.present(dcdg2u).and.present(dcdg2d).and.present(wxup).and.present(wxdn)&
   .and.present(wcup).and.present(wcdn)) then
...
! potential-only functional
      call xcifc_libxc(xctype,n,c_tb09=c_tb09,rhoup=rhoup,rhodn=rhodn, &
       g2up=g2up,g2dn=g2dn,gup2=gup2,gdn2=gdn2,gupdn=gupdn,tauup=tauup, &
       taudn=taudn,vxup=vxup,vxdn=vxdn,vcup=vcup,vcdn=vcdn,dxdgu2=dxdgu2, &
...
! spin-polarised kinetic energy functional
    call xcifc_libxc(xctype,n,rhoup=rhoup,rhodn=rhodn,g2up=g2up,g2dn=g2dn, &
...
! spin-polarised LSDA
   ...
   .and.present(vx).and.present(vc).and.present(dxdgr2).and.present(dcdgr2) &
   .and.present(dxdg2r).and.present(dcdg2r).and.present(wx).and.present(wc))then
...
! potential-only functional
      call xcifc_libxc(xctype,n,c_tb09=c_tb09,rho=rho,g2rho=g2rho,grho2=grho2, &
       tau=tau,vx=vx,vc=vc,dxdgr2=dxdgr2,dcdgr2=dcdgr2,dxdg2r=dxdg2r, &
       dcdg2r=dcdg2r,wx=wx,wc=wc)
    end if
...
! spin-unpolarised GGA
   ...
   .and.present(wx)) then
! spin-unpolarised kinetic energy functional
...
case(50)
  ...
  xcgrad=0
case(51)
  xcdescr='Thomas-Fermi kinetic energy functional scaled by the ratio of the &
   &exact τ to the TF τ'
  xcspin=-1
  xcgrad=0
case(52)
