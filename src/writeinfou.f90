...
! local variables
real(8) t1
call writebox(fnum,"Ultra long-range calculation")
write(fnum,*)
...
if (trdvclr) then
  write(fnum,*)
  write(fnum,'("Real-space external Coulomb potential read from VCLR.OUT")')
end if
if (trdbfcr) then
  write(fnum,*)
  write(fnum,'("Real-space external magnetic field in Cartesian coordinates &
   &read from BFCR.OUT")')
end if
if (fsmtype /= 0) then
  write(fnum,*)
