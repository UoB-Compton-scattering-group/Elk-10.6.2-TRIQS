...
if(task == 808) then
  ...
  do ik=1,nkpt
    evalsum=evalsum+wkpt(ik)*sum(occsv(1:nstsv,ik)*evalsv(1:nstsv,ik))
  end do
end if
!end edit
!------------------------!
...
! kinetic energy from valence states
  ...
  do ik=1,nkpt
    ...
    do ist=1,nstsv
      ...
      engykn=engykn+w*occsv(ist,ik)*z1%re
    end do
  end do
...
! remove vector potential energy term if required
if (tafield.and.tlast) then
  engykn=engykn+(0.5d0/solsc)*dot_product(afieldc(:),jtot(:))
end if
! remove spin-dependent vector potential energy term if required
if (tafsp.and.tlast) call energyafsp
!-------------------------------!
