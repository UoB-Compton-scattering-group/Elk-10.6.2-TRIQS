...
if (lp_mpi == 0) then
  mp_mpi=.true.
  write(str,'("Elk code version ",I0,".",I0,".",I0," started")') version
  call writebox(6,trim(str))
else
  mp_mpi=.false.
end if
10 continue
...
if (ramdisk) then
  ...
  if (mp_mpi) then
    ...
    if (.not.wrtdsk) then
      write(*,*)
      write(*,'("Info(elk): some direct access files are not written to disk")')
    end if
  end if
else
  wrtdsk=.true.
...
do itask=1,ntasks
  task=tasks(itask)
  if (mp_mpi) then
    write(str,'("Current task : ",I0)') task
    call writebox(6,trim(str))
  end if
...
if (mp_mpi) then
  ...
  call writebox(6,"Elk code stopped")
end if
...
!BOI
! !TITLE: {\huge{\sc The Elk Code Manual}}\\ \Large{\sc Version 10.6.2}\\ \vskip 20pt \includegraphics[height=1cm]{elk_silhouette.pdf}
...
!   Yunfan Liang, Xavier Gonze, Mike Bruckhoff, Eddie Harris-Lee, Andreas
!   Fischer, Wenhan Chen and Jyoti Krishna. Special mention of David Singh's
!   very useful book on the LAPW method\footnote{D. J. Singh, {\it Planewaves,
...
!   Lars Nordstr\"{o}m, Francesco Cricchio, Oscar Gr\aa n\"{a}s \\
!   Hardy Gross
!
!   \vspace{12pt}
...
!   Libxc is a library of exchange-correlation functionals. Elk can use the
!   complete set of LDA and GGA functionals available in Libxc as well as some
!   meta-GGAs. In order to enable this, first download and compile Libxc. This
!   should have produced the files {\tt libxc.a} and {\tt libxcf03.a} in the
!   Libxc directory {\tt src/.libs}. Copy these to the {\tt elk/src} directory
!   and then uncomment the lines indicated for Libxc in {\tt elk/make.inc}. Once
...
!   {\tt elk/src/libxcf03.f90} for a list of the functionals and their
!   associated numbers.
!
...
!
!   \block{avecref}{
!   {\tt avecref(1)} & first reference lattice vector, etc. & real(3) &
!    $(0.0,0.0,0.0)$}
!   Reference lattice vectors for calculating the ${\bf G}$-vector grid and
!   derived quantities. If any of these elements are non-zero then the code
!   computes the corresponding reciprocal lattice vectors and set of
!   ${\bf G}$-vectors. These are then transformed to be identical to those
!   calculated with {\tt avec}. The purpose of this is to enable accurate
!   energy-volume curves, where the number of grid points should remain fixed
!   for all volumes.
!
!   \block{beta0}{
...
!
!   \block{bfdmag}{
!   {\tt bfdmag} & {\tt .true.} if the external ${\bf B}$-field diamagnetic
!    coupling term should be included & logical & {\tt .false.}}
!   This causes the diamagnetic coupling term
!   $$ H_{\rm dia}=\frac{B^2 r^2}{8c^2}\big(1-(\hat{\bf B}\cdot
!    \hat{\bf r})^2\big) $$
!   to be included in the first-variational Hamiltonian. Note that because this
!   is a scalar potential, spin-polarisation does not have to be enabled for
!   this term to have an effect.
!
!   \block{bfieldc}{
...
!   $$ \frac{g_e}{4c}\,\vec{\sigma}\cdot{\bf B}, $$
...
!
!   \block{bfieldcu}{
!   {\tt bfieldcu} & global external magnetic field in Cartesian coordinates &
!    real(3) & $(0.0,0.0,0.0)$}
!   As with {\tt bfieldc} but applied only in the ultracell during an ultra
!   long-range calculation.
!
!   \block{bforb}{
!   {\tt bforb} & {\tt .true.} if the external ${\bf B}$-field-orbit coupling
!    term should be included & logical & {\tt .false.}}
!   This causes the term corresponding to the coupling between an external
!   magnetic field and the orbit of an electron (orbital paramagnetism)
!   $$ \hat{H}_{{\bf B}{\rm o}}=\frac{1}{2c}{\bf B}\cdot\hat{\bf L} $$
!   to be added to the second-variational Hamiltonian. Spin-polarisation is
...
!
!   \block{fsmtype}{
!   {\tt fsmtype} & fixed spin moment (FSM) type & integer & 0}
!   The magnetic moment, its direction or magnitude can be fixed both globally
!   as well as in each muffin-tin individually. The options are as follows:
!   \vskip 6pt
!   \begin{tabularx}{\textwidth}[h]{lX}
!   0 & no FSM \\
...
!   \end{tabularx}
!   \vskip 6pt
!   See also {\tt momfix}, {\tt momfixm}, {\tt mommtfix}, {\tt mommtfixm},
!   {\tt taufsm} and {\tt spinpol}.
!
!   \block{ftmtype}{
...
!
!   \block{mbwgrf}{
!   {\tt mbwgrf} & matrix bandwidth of response functions in the
!    ${\bf G}$-vector basis & integer & $-1$}
!   Setting this to a positive integer results in response functions and the
!   screened interaction $W({\bf G},{\bf G}',{\bf q},\omega)$ being treated as a
!   banded matrix in ${\bf G}$ and ${\bf G}'$ with bandwidth {\tt mbwgrf}.
!   This can be used to speed up $GW$ calculations.
!
!   \block{mixsave}{
!   {\tt mixsave} & {\tt .true.} if the mixer work array is to be saved during a
!    ground-state run & logical & {\tt .false.}}
!   If {\tt mixsave} is {\tt .true.}, then the mixer work array is saved to
!   {\tt MIXWORK.OUT} every {\tt nwrite} iterations and at the end of the
!   self-consistent loop. This array is subsequently read in at the beginning of
!   a restarted calculation in order to improve convergence.
!
!   \block{mixtype}{
...
!    logical & {\tt .false.}}
!   If {\tt molecule} is {\tt .true.}, then the atomic positions given in the
!   {\tt atoms} block are assumed to be in Cartesian coordinates.
!
!   \block{momfix}{
...
!   calculations). Applies when {\tt fsmtype} is 1(-1) or 3(-3). See
!   {\tt fsmtype}, {\tt taufsm} and {\tt spinpol}.
!
!   \block{momfixm}{
!   {\tt momfixm} & the desired total moment magnitude for a FSM calculation &
!    real & $0.0$}
!   This applies when {\tt fsmtype} is 4 or 6.
!
!   \block{mommtfix}{
!   {\tt is} & species number & integer & 0 \\
!   {\tt ia} & atom number & integer & 0 \\
!   {\tt mommtfix} & desired muffin-tin moment for a FSM calculation &
!    real(3) & $(0.0,0.0,0.0)$}
...
!   specified (even for collinear calculations). Applies when {\tt fsmtype} is
!   2(-2) or 3(-3). Note that the moment is not fixed in a particular muffin-tin
!   when the magnitude of any component of the corresponding {\tt mommtfix} is
!   $\ge 1000$. See {\tt fsmtype}, {\tt taufsm} and {\tt spinpol}.
!
!   \block{mommtfixm}{
!   {\tt is} & species number & integer & 0 \\
!   {\tt ia} & atom number & integer & 0 \\
!   {\tt mommtfixm} & desired muffin-tin moment magnitude for a FSM
!    calculation & real & $-1$}
!   This applies when {\tt fsmtype} is 5 or 6. Note that the moment magnitude is
!   not fixed in a particular muffin-tin when the corresponding {\tt mommtfixm}
!   is negative.
!
!   \block{mrmtav}{
...
!
!   \block{q0cut}{
!   {\tt q0cut} & Q-vector cut-off for the ultra long-range Coulomb Green's
!    function & real & $0.0$}
!   The ultra long-range Poisson equation is solved using the Green's function
!   $4\pi/|{\bf G}+{\bf Q}|^2$. This is set to zero for all
!   $|{\bf G}+{\bf Q}|<{\tt q0cut}$ when {\tt q0cut} is positive. For negative
!   {\tt q0cut}, a Yukawa-type screening of the form
!   $4\pi/(|{\bf G}+{\bf Q}|^2+{\tt q0cut}^2)$ is employed. Setting this
!   variable to be small but finite can improve the stability of a
...
!
!   \block{rmtall}{
!   {\tt rmtall} & muffin-tin radius for all species & real & $-1.0$}
!   If {\tt rmtall} is positive then all muffin-tin radii are set to this value.
!
!   \block{rmtdelta}{
!   {\tt rmtdelta} & minimum allowed distance between muffin-tin surfaces &
!    real & $0.05$}
!
!   \block{rmtscf}{
!   {\tt rmtscf} & muffin-tin radius scaling factor & real & $1.0$}
!   All muffin-tin radii read from the species files are scaled by this factor.
!
!   \block{rndavec}{
!   {\tt rndavec} & lattice vector randomisation amplitude & real & $0.0$}
!   Setting this to a number larger than zero causes the code add random jitter
!   to the lattice vectors proportional to this amplitude. This can be used to
...
!    {\tt vklem}. \\
!   31/2/3 & 1/2/3D charge density plot. \\
!   41/2/3 & 1/2/3D exchange-correlation and Coulomb potential plots. \\
!   51/2/3 & 1/2/3D electron localisation function (ELF) plot. \\
!   61/2/3 & 1/2/3D wavefunction plot:
!    $\left|\Psi_{i{\bf k}}({\bf r})\right|^2$. \\
!   65 & Write the core wavefunctions to file for plotting. \\
!   68 & Output the status of the RAM disk. \\
!   71/2/3 & 1/2/3D plot of magnetisation vector field, ${\bf m}({\bf r})$. \\
!   81/2/3 & 1/2/3D plot of exchange-correlation magnetic vector field,
!    ${\bf B}_{\rm xc}({\bf r})$. \\
!   91/2/3 & 1/2/3D plot of $\nabla\cdot{\bf B}_{\rm xc}({\bf r})$. \\
!   100 & 3D Fermi surface plot using the scalar product
!    $p({\bf k})=\prod_i(\epsilon_{i{\bf k}}-\epsilon_{\rm F})$. \\
...
!    $\langle\Psi_{i{\bf k+q}}|e^{i{\bf q}\cdot{\bf r}}|
!    \Psi_{j{\bf k}}\rangle$.
!   \end{tabularx}
!
!   \begin{tabularx}{\textwidth}[h]{lX}
!   135 & Output all wavefunctions expanded in the plane wave basis up to a
!    cut-off defined by {\tt hkmax}. \\
!   140 & Energy loss near edge structure (ELNES). \\
!   141/2/3 & 1/2/3D plot of the electric field
!    ${\bf E}({\bf r})\equiv\nabla V_{\rm C}({\bf r})$. \\
!   150 & Write out the atomic eigenvalues for each species. \\
!   151/2/3 & 1/2/3D plot of
!    ${\bf m}({\bf r})\times{\bf B}_{\rm xc}({\bf r})$. \\
!   160 & Calculates the total exchange-correlation spin-torque acting on the
!    system: $\boldsymbol\tau=\int d^3r\, {\bf m}({\bf r})\times
!    {\bf B}_{\rm xc}({\bf r}).$ \\
!   162 & 2D scanning-tunneling microscopy (STM) image. \\
!   170 & Writes the electron momentum density to {\tt EMD.OUT}. \\
!   171/2/3 & 1/2/3D plot of the electron momentum density. \\
!   180 & Generate the RPA inverse dielectric function with local contributions
!    $\epsilon^{-1}({\bf G},{\bf G}',{\bf q},\omega)$ and write it to file. \\
...
!   202 & Phonon dry run: just produce a set of empty {\tt DYN} files. \\
!   205 & Calculation of phonon dynamical matrices using density functional
!    perturbation theory (DFPT). \\
!   208 & Calculation of static Born effective charges. \\
!   209 & Born effective charge dry run: produce a set of empty {\tt BEC}
!    files. \\
!   210 & Phonon density of states. \\
!   220 & Phonon dispersion plot. \\
!   230 & Phonon frequencies and eigenvectors for an arbitrary $q$-point. \\
!   240/1 & Generate the ${\bf q}$-dependent phonon linewidths and
!    electron-phonon coupling constants and write them to file. Task 241 also
!    writes the complete set of electron-phonon coupling matrix elements to
!    {\tt EPHMAT.OUT}. \\
...
!    $T_c$. \\
!   260 & Solves the Eliashberg equations to find the superconducting gap. \\
!   270 & Electron-phonon Bogoliubov equation ground-state. \\
!   271 & Resumption of the Bogoliubov ground-state. \\
!   280 & Electron-phonon Bogoliubov density of states. \\
!   285 & Electron and phonon anomalous correlation entropy (ACE). \\
...
!   330/1 & TDDFT calculation of the spin-polarised response function for
!    arbitrary ${\bf q}$-vectors. Task 331 writes the entire response function
!    $\overleftrightarrow{\chi}({\bf G},{\bf G}',{\bf q},\omega)$ to file.
!   \end{tabularx}
!
!   \begin{tabularx}{\textwidth}[h]{lX}
!   341/2/3 & 1/2/3D plot of
!    $w_{\rm xc}({\bf r})\equiv \left.\delta E_{\rm xc}[\rho,\tau]/
!    \delta \tau({\bf r})\right|_{\rho}$ which is calculated for meta-GGA
!    functionals. \\
!   350/1/2 & Spin-spiral supercell calculations (spin-orbit coupling can be
!    included). Task 352 is a dry run: produce a set of empty {\tt SS} files. \\
!   371/2/3 & 1/2/3D plot of the paramagnetic current density
!    ${\bf j}_{\rm p}({\bf r})$. \\
!   380 & Piezoelectric tensor. \\
!   390 & Magnetoelectric tensor. \\
!   400 & Calculation of tensor moments and corresponding DFT+$U$ Hartree-Fock
!    energy contributions. \\
!   420/1 & Molecular dynamics (MD) calculation within the adiabatic
!    approximation. Task 421 restarts an interrupted MD calculation. \\
!   430 & Write the strain tensors to {\tt STRAIN.OUT}. \\
!   440 & Write the stress tensor components corresponding to the strain tensors
!    to {\tt STRESS.OUT}. \\
!   450 & Generates a laser pulse in the form of a time-dependent vector
!    potential ${\bf A}(t)$ and writes it to {\tt AFIELDT.OUT}. \\
!   455 & Writes the time-dependent power density and total energy density of
!    ${\bf A}(t)$. \\
!   456 & Writes the frequency-dependent electric field ${\bf E}(\omega)$
!    corresponding to ${\bf A}(t)$. \\
!   460/1/2/3 & Time evolution run using TDDFT under the influence of
!    ${\bf A}(t)$. Tasks 462 and 463 include nuclear Ehrenfest dynamics. Tasks
!    461 and 463 restart interrupted calculations. \\
!   471 & 1D plot of the static charge density. \\
!   478 & Calculation of the dynamical Born effective charges. \\
!   480/1 & Computes the dielectric function from the time-dependent current
!    density in {\tt JTOT\_TD.OUT}. Task 481 assumes that the vector potential
!    ${\bf A}(t)$ is a step function at $t=0$, while task 480 makes no such
!    assumptions. \\
!   500 & Checks the test files generated when {\tt test} is {\tt .true.} \\
!   550 & Writes the files required by Wannier90. \\
!   600 & Output the $GW$ self-energy matrix elements. \\
!   610 & Generates the $GW$ spectral function. \\
!   620 & Generates the $GW$ band structure, i.e. the ${\bf k}$-dependent
!    spectral function). \\
!   640 & Determines the $GW$ density matrix in terms of natural orbitals and
!    occupation numbers. The files {\tt EVECSV.OUT} and {\tt OCCSV.OUT} are
!    overwritten by these and can then be used for other calculations. \\
!   700/1 & Ultra long-range ground-state calculation. Task 701 is for
!    restarting from the potential in {\tt STATE\_ULR.OUT}. \\
!   731/2/3 & 1/2/3D plot of the ultra long-range density. \\
!   741/2/3 & 1/2/3D plot of the ultra long-range Kohn-Sham potential. \\
!   771/2/3 & 1/2/3D plot of the ultra long-range magnetisation.
!   \end{tabularx}
!
!   \block{tau0atp}{
...
!   DFT+$U$ calculations. Alternatively, with {\tt task} equal to 400, one can
...
!
!   \block{trdbfcr}{
!   {\tt trdbfcr} & set to {\tt .true.} if the ultra long-range, real-space
!    external magnetic field in Cartesian coordinates should be read in from
!    {\tt BFCR.OUT} & logical & {\tt .false.}}
!
!   \block{trdvclr}{
!   {\tt trdvclr} & set to {\tt .true.} if the ultra long-range, real-space
!    external Coulomb potential should be read in from {\tt VCLR.OUT} &
!    logical & {\tt .false.}}
!
!   \block{tsediag}{
...
!    \begin{verbatim}
!     write(*,*)
!     write(*,'("Error(readinput): natoms < 1 : ",I8)') natoms(is)
