...
deallocate(lock)
! add χ₀ from each process and redistribute
if (np_mpi > 1) then
  n=ngrf*4*ngrf*4*nwrf
  call mpi_allreduce(mpi_in_place,chi,n,mpi_double_complex,mpi_sum,mpicom, &
   ierror)
end if
! transform χ₀ from 2x2 to 1x3 basis
do iw=1,nwrf
  do ig=1,ngrf
    do jg=1,ngrf
      a(1:4,1:4)=chi(ig,1:4,jg,1:4,iw)
      call tfm2213(a,b)
      chi(ig,1:4,jg,1:4,iw)=b(1:4,1:4)
    end do
  end do
end do
! generate transverse χ₀ for the collinear case
if (.not.ncmag) then
  allocate(chit(nwrf))
  do iw=1,nwrf
    a(1:4,1:4)=chi(1,1:4,1,1:4,iw)
    call tfm13t(a,b)
    chit(iw)=b(2,2)
  end do
end if
...
! write transverse χ₀ for collinear case
  if (.not.ncmag) then
...
do iw=1,nwrf
! multiply f_Hxc by -χ₀ from the left
  z1=-1.d0
...
! invert the matrix
  call zminv(n,c)
! multiply by χ₀ on the right and store in χ
...
if (mp_mpi) then
! write the complete χ matrix if required
  if (task == 331) then
    open(120,file='CHI.OUT',form='UNFORMATTED',action='WRITE')
    write(120) chi
    close(120)
  end if
! write χ for G = G' = 0 in the 1x3 basis
  do i=1,4
    do j=1,4
...
! write transverse χ for collinear case
  ...
  write(*,*)
  write(*,'(" Non-interacting Kohn-Sham response function written to &
   &CHI0_ij.OUT")')
  if (.not.ncmag) then
    write(*,*)
...
do i=1,4
  c(i,1)=a(i,1)+a(i,4)
  c(i,2)=a(i,2)+a(i,3)
  c(i,3)=zmi*(a(i,2)-a(i,3))
  c(i,4)=a(i,1)-a(i,4)
end do
do j=1,4
  b(1,j)=c(1,j)+c(4,j)
  b(2,j)=c(2,j)+c(3,j)
  b(3,j)=zi*(c(2,j)-c(3,j))
  b(4,j)=c(1,j)-c(4,j)
end do
end subroutine
