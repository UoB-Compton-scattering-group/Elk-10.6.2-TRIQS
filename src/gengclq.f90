...
implicit none
! local variables
integer, parameter :: np=5,ns0=10,nss=20
...
! subdivision vectors in lattice coordinates
    d(1:3)=1.d0/dble(ngridq(1:3)*2*ns)
! compute the integral of 1/q²
    sm=0.d0
    do i1=-ns,ns-1
      t1=dble(i1)*d(1)
      v1(1:3)=vqc(1:3,iq)+t1*bvec(1:3,1)
      do i2=-ns,ns-1
        t1=dble(i2)*d(2)
        v2(1:3)=v1(1:3)+t1*bvec(1:3,2)
        do i3=-ns,ns-1
          t1=dble(i3)*d(3)
          v3(1:3)=v2(1:3)+t1*bvec(1:3,3)
          t2=v3(1)**2+v3(2)**2+v3(3)**2
          if (t2 > 1.d-14) sm=sm+1.d0/t2
        end do
      end do
    end do
    t1=1.d0/dble(2*ns)
...
call writetest(700,"regularised Coulomb Green''s function (gclq)",nv=nqpt, &
 tol=1.d-8,rva=gclq)
end subroutine
!EOC
