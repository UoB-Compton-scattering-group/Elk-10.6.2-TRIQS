...
!   g0   : major component multiplied by r (out,real(nr))
!   g1   : radial derivative of g0 (out,real(nr))
!   f0   : minor component multiplied by r (out,real(nr))
...
! set the remaining points and return
    g0(ir+1:nr)=g0(ir); g1(ir+1:nr)=g1(ir)
    f0(ir+1:nr)=f0(ir); f1(ir+1:nr)=f1(ir)
    return
  end if
! check for node
