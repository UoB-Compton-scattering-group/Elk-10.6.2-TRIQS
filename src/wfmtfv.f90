...
do ilo=1,nlorb(is)
  ...
  do lm=l**2+1,(l+1)**2
    i=npci+lm
    z=evecfv(ngp+idxlo(lm,ilo,ias))
    if (abs(z%re)+abs(z%im) > 1.d-10) then
      if (l <= lmaxi) call zfzrf(nrci,lofr(1,1,ilo,ias),lmmaxi,wfmt(lm))
      call zfzrf(nrco,lofr(iro,1,ilo,ias),lmmaxo,wfmt(i))
    end if
  end do
end do
!-----------------------!
...
do l=0,lmaxo
  do lm=l**2+1,(l+1)**2
    i=npci+lm
    do io=1,apword(l,is)
      z=zdotu(ngp,evecfv,1,apwalm(:,io,lm),1)
      if (abs(z%re)+abs(z%im) > 1.d-10) then
        if (l <= lmaxi) call zfzrf(nrci,apwfr(1,1,io,l,ias),lmmaxi,wfmt(lm))
        call zfzrf(nrco,apwfr(iro,1,io,l,ias),lmmaxo,wfmt(i))
      end if
    end do
  end do
end do
