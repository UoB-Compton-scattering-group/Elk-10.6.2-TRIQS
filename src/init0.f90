...
! ADNJ - added ELFS, DFT+DMFT density, and DFT+DMFT density matrix 
if (any(task == [5,10,51,52,53,170,300,460,461,462,463,600,601,620,700,701,720,808,809]).or. &
 ksgwrho) then
  tevecsv=.true.
end if
...
if (spinpol) n=n+(npmtmax*natmtot+ngtot)*ndmag
allocate(rhmg(n))
...
if (dftu /= 0) then
  if (any(task == [5,300,600,601,610,620,640])) then
    write(*,*)
