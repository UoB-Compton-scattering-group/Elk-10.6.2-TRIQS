...
integer ntemp
! dynamical matrices for all q-points
complex(8), allocatable :: dynq(:,:,:)
! dynamical matrices for all R-points
real(8), allocatable :: dynr(:,:,:)
! phonon frequencies for all q-points
real(8), allocatable :: wphq(:,:)
! tphnat is .true. if the non-analytic term is to be added to the phonon
! dispersion; this requires the Born effective charges and dielectric function
logical tphnat
...
! muffin-tin Kohn-Sham potential derivative
complex(8), pointer, contiguous :: dvsmt(:,:)
! interstitial Kohn-Sham potential derivative
complex(8), allocatable :: dvsir(:)
! G+q-space interstitial Kohn-Sham potential derivative
complex(8), pointer, contiguous :: dvsig(:)
! Kohn-Sham effective magnetic field derivative
complex(8), pointer, contiguous :: dbsmt(:,:,:),dbsir(:,:)
